...
// BuildArgs constructs stateless CLI arguments for agy execution.
func (r *Runner) BuildArgs(prompt string) []string {
	return []string{"--dangerously-skip-permissions", "--output-format", "json", "-p", prompt}
}

// Execute runs the prompt against agy statelessly and returns the generated reply text.
func (r *Runner) Execute(ctx context.Context, prompt string) (string, error) {
	...
	if startIdx >= 0 && endIdx > startIdx {
		...
		if err := json.Unmarshal([]byte(jsonStr), &cliRes); err == nil && cliRes.Response != "" {
			return CleanOutput(cliRes.Response), nil
		}
	}
